...
def test_pdfplumber_success():
    ...
    with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as f:
        f.write(b"%PDF-1.4 minimal"); path = f.name
    try:
...
def test_pymupdf_fallback():
    from parser.extractor import extract_text
    mock_page = MagicMock()
    mock_page.get_text.return_value = "Passport No: J8821045 " * 10
...
def test_truncation():
    from parser.extractor import extract_text, MAX_CHARS
